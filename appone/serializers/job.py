...
class JobApplicationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = '__all__'
        read_only_fields = (
            'id', 'freelancer', 'job', 'status', 'applied_at', 'updated_at'
        )
...
class HireFreelancerSerializer(serializers.Serializer):
    ...
    monthly_rate = serializers.DecimalField(max_digits=10, decimal_places=2, required=True)
    start_date = serializers.DateField(required=True)


class UpdateJobApplicationStatusSerializer(serializers.Serializer):
    """Serializer for updating job application status."""
    status = serializers.ChoiceField(choices=JobApplication.STATUS_CHOICES, required=True)

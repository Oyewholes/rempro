...
class FreelancerProfileViewSet(viewsets.ModelViewSet):
    ...
    def me(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                FreelancerProfileSerializer(profile, context={'request': request}).data
            )
    ...
    @extend_schema(
        ...
        description=(
            'Checks that the freelancer is physically located in Nigeria by resolving '
            'their IP address via ipapi.co with fallback to ip-api.com.'
        ),
        responses={
    ...
    @extend_schema(
        summary='Upload CV',
        description='Upload a PDF/Word CV file. Processed asynchronously via Celery.',
        responses={
    ...
    def upload_cv(self, request):
        ...
        return _handle_file_upload(
            self, request,
            file_key='cv_file', file_type='cv',
            profile_field='cv_file', allowed_types=_CV_ALLOWED_CONTENT_TYPES,
        )

    @extend_schema(
        summary='Upload live photo',
        description='Upload a JPEG/PNG live photo. Processed asynchronously via Celery.',
        responses={
            202: OpenApiResponse(description='Upload accepted and queued.'),
            400: OpenApiResponse(description='Missing or invalid file.'),
        },
    ...
    def upload_live_photo(self, request):
        ...
        return _handle_file_upload(
            self, request,
            file_key='live_photo', file_type='live_photo',
            profile_field='live_photo', allowed_types={'image/jpeg', 'image/png'},
        )
    ...
    def add_nin(self, request):
        ...
        profile.calculate_profile_completion()
        return Response({'message': 'NIN added successfully'}, status=status.HTTP_200_OK)
    ...
    def add_portfolio(self, request):
        ...
        return Response(
            {'message': 'Portfolio item added successfully', 'portfolio': profile.portfolio_files},
            status=status.HTTP_200_OK,
        )
    ...
    def add_banking_details(self, request):
        ...
        profile.save(update_fields=['paystack_email', 'payoneer_email', 'bank_details_verified'])
        profile.calculate_profile_completion()

        return Response({'message': 'Banking details added successfully'}, status=status.HTTP_200_OK)
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter('digital_id', str, description='The freelancer\'s digital UUID'),
        ],
        responses={
    ...
    def generate_id_card(self, request):
        ...
        try:
            generate_id_card_task.apply_async(args=[str(profile.id)], ignore_result=False)
        except Exception as exc:
            logger.error(
                'generate_id_card: failed to enqueue task for profile %s: %s', profile.id, exc
            )
            return Response(
